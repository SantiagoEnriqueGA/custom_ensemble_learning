...
class RandomForest(object):
    ...
    def fitting(self):
        ...
        with multiprocessing.Pool() as pool:
            # Zip decision trees, datasets, and labels
            jobs = [(self.decision_trees[i], self.bootstraps_datasets[i], self.bootstraps_labels[i])
                    for i in range(self.num_trees)]
            # Parallelize the fitting process
            self.decision_trees = pool.starmap(fit_tree, jobs)
